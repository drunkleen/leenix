local M = {}
local g = vim.g
local opts = require("tiredconfig").base46
...
M.toggle_theme = function()
  ...
  if opts.theme ~= themes[1] and opts.theme ~= themes[2] then
    vim.notify "Set your current theme to one of those mentioned in the theme_toggle table (tiredvim)"
    return
  end
  ...
  package.loaded.tiredvim = nil
  local tiredvim = require "tiredvim"
  local old_theme = tiredvim.base46.theme

  require("tired.utils").replace_word('theme = "' .. old_theme, 'theme = "' .. opts.theme)
...
M.toggle_transparency = function()
  ...
  package.loaded.tiredvim = nil
  local old = require("tiredvim").base46.transparency
  local new = "transparency = " .. tostring(opts.transparency)
  require("tired.utils").replace_word("transparency = " .. tostring(old), new)
end
