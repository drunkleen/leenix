...
local ui = require "tired.themes.ui"
local state = require "tired.themes.state"
local colors = dofile(vim.g.base46_cache .. "colors")

state.ns = api.nvim_create_namespace "NvThemes"

if not state.val then
  state.val = require("tired.utils").list_themes()
  ...
  state.close_opts = {
    ...
    after_close = function()
      if not state.confirmed then
        require("plenary.reload").reload_module "tiredvim"
        local theme = require("tiredvim").base46.theme
        require("tired.themes.utils").reload_theme(theme)
      end
      require("plenary.reload").reload_module "tired.themes"
      vim.cmd.stopinsert()
    end,
  }

  volt.mappings(state.close_opts)

  require "tired.themes.mappings"
