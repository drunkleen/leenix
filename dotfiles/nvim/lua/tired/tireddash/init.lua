...
local opts = require("tiredconfig").tireddash
...
M.open = function(buf, win, action)
  ...
  local ns = api.nvim_create_namespace "tireddash"
  local winh = api.nvim_win_get_height(win)
  local winw = api.nvim_win_get_width(win)
  buf = buf or vim.api.nvim_create_buf(false, true)

  vim.g.tireddash_buf = buf
  vim.g.tireddash_win = win
  ...
  for _, v in ipairs(opts.header) do
    ...
    if headerw > tireddash_w then
      tireddash_w = headerw
    end

    local col = math.floor((winw / 2) - math.floor(strw(v) / 2)) - 6
    local opt = { virt_text_win_col = col, virt_text = { { v, "TiredDashAscii" } } }
    table.insert(ui, opt)
  end
  ...
  for i, v in ipairs(opts.buttons) do
    ...
    if v.multicolumn then
      ...
      col = math.floor((winw / 2) - math.floor(w / 2)) - 6
      opt = { virt_text_win_col = col, virt_text = { { str, v.hl or "TiredDashButtons" } } }
    end
  ...
  require("tired.utils").set_cleanbuf_opts("tireddash", buf)

  ----------------------- autocmds -----------------------------
  local group_id = api.nvim_create_augroup("TiredDashAu", { clear = true })

  api.nvim_create_autocmd("BufWinLeave", {
    group = group_id,
    buffer = buf,
    callback = function()
      vim.g.tireddash_displayed = false
      api.nvim_del_augroup_by_name "TiredDashAu"
    end,
  })
  ...
  api.nvim_create_autocmd({ "WinResized", "VimResized" }, {
    group = group_id,
    callback = function()
      vim.bo[vim.g.tireddash_buf].ma = true
      require("tired.tireddash").open(vim.g.tireddash_buf, vim.g.tireddash_win, "redraw")
    end,
  })
end
