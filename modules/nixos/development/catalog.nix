...
#   heavy            large / slow / memory-heavy toolchain (informational)
{
  ...
  libraries = {
    ...
    icu           = { description = "ICU (icu4c)"; kind = "library"; classification = "A"; packages = pkgs: [ pkgs.icu ]; platforms = [ "x86_64-linux" "aarch64-linux" ]; guarded = false; unfree = false; heavy = false; };
