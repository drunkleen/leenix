  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Share clipboard, files, or folders with LocalSend

        # leenix:group=share

        # leenix:name=

        # leenix:args=<clipboard|file|folder> [path...]

        # leenix:examples=leenix share clipboard | leenix share file ~/Downloads/example.txt

        if (($# == 0)); then
          echo "Usage: leenix-menu-share [clipboard|file|folder]"
          exit 1
        fi
