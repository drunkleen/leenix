...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-windows-vm";
      excludeShellChecks = [ "SC2001" "SC2086" ];
      ...
      text = ''
        ...
        install_windows() {
          ...
          leenix-pkg-add freerdp openbsd-netcat gum
          ...
          if [[ -f $LEENIX_PATH/applications/icons/windows.png ]]; then
            cp "$LEENIX_PATH/applications/icons/windows.png" "$HOME/.local/share/applications/icons/windows.png"
          fi
        ...
        Exec=uwsm app -- leenix-windows-vm launch
        ...
        EOF
          ...
          echo ""
          echo "To stop the VM: leenix-windows-vm stop"
        ...
        launch_windows() {
          ...
          if [[ ! -f $COMPOSE_FILE ]]; then
            echo "Windows VM not configured. Please run: leenix-windows-vm install"
            exit 1
          fi
          ...
          CONTAINER_STATUS=$(docker inspect --format='{{.State.Status}}' leenix-windows 2>/dev/null)
          ...
          if [[ $CONTAINER_STATUS != "running" ]]; then
            ...
            if ! docker-compose -f "$COMPOSE_FILE" up -d 2>&1; then
              echo "❌ Failed to start Windows VM!"
              echo "   Try checking: leenix-windows-vm status"
              echo "   View logs: docker logs leenix-windows"
            ...
            WAIT_COUNT=0
            until docker logs leenix-windows 2>&1 | grep -qi "windows started successfully"; do
              sleep 2
              WAIT_COUNT=$((WAIT_COUNT + 1))
              if (( WAIT_COUNT > 60 )); then  # 2 minutes timeout
                echo ""
                echo "❌ Timeout: Windows VM failed to start within 2 minutes"
                echo "   Check logs: docker logs leenix-windows"
                exit 1
              fi
            done
          fi
        ...
        To stop: leenix-windows-vm stop"
          else
          ...
          xfreerdp3 /u:"$WIN_USER" /p:"$WIN_PASS" /v:127.0.0.1:3389 -grab-keyboard /sound /microphone /clipboard /cert:ignore /title:"Windows VM - Leenix" /dynamic-resolution /gfx:AVC444 /floatbar:sticky:off,default:visible,show:fullscreen $RDP_SCALE
          ...
          if [[ $KEEP_ALIVE = "false" ]]; then
            ...
            echo "To stop it: leenix-windows-vm stop"
          fi
        }
        ...
        status_windows() {
          if [[ ! -f $COMPOSE_FILE ]]; then
            echo "Windows VM not configured."
            echo "To set up: leenix-windows-vm install"
            exit 1
          fi
          ...
          if [[ -z $CONTAINER_STATUS ]]; then
            ...
            echo "To start: leenix-windows-vm launch"
            ...
            gum style \
              ...
              "" \
              "To connect: leenix-windows-vm launch" \
              "To stop:    leenix-windows-vm stop"
          else
        ...
        show_usage() {
          echo "Usage: leenix-windows-vm [command] [options]"
          echo ""
          echo "Commands:"
          ...
          echo ""
          echo "Examples:"
          echo "  leenix-windows-vm install           # Set up Windows VM for first time"
          echo "  leenix-windows-vm launch            # Connect to VM (auto-stop on exit)"
          echo "  leenix-windows-vm launch -k         # Connect to VM (keep running)"
          echo "  leenix-windows-vm stop              # Shut down the VM"
        }
