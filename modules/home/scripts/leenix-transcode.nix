  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:group=transcode

        # leenix:name=

        # leenix:summary=Transcode pictures and videos for sharing

        # leenix:args=[--path path] [input] [format] [resolution]

        # leenix:examples=leenix transcode|leenix transcode --path ~/Downloads|leenix transcode ~/Videos/demo.mov mp4 1080p|leenix transcode ~/Pictures/wallpaper.heic jpg medium
        ...
        usage() {
          cat <<'EOF'
        Usage:
        leenix transcode [--path path] [input] [format] [resolution]
        ...
        main() {
          ...
          if [[ -z $input ]]; then
            input=$(leenix-menu-file \
              "Transcode picture or video" \
              "$search_path" \
              "jpg jpeg png webp gif heic avif mp4 mov m4v mkv webm avi")
          fi
          ...
          if [[ -z $format ]]; then
            if [[ $type == "picture" ]]; then
              format=$(leenix-menu-select "Select format" jpg png)
            else
              format=$(leenix-menu-select "Select format" mp4 gif)
            fi
          fi

          if [[ -z $resolution ]]; then
            if [[ $type == "picture" ]]; then
              resolution=$(leenix-menu-select "Select resolution" high medium low)
            else
              resolution=$(leenix-menu-select "Select resolution" 4k 1080p 720p)
            fi
          fi
          ...
          if [[ $type == "video" ]]; then
            leenix-notification-send \
              "" \
              "Transcoding video…" \
              "$(basename -- "$input") to $format ($resolution)"
            ...
            leenix-notification-send \
              "" \
              "Transcoded to $resolution $format" \
