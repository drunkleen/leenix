  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Ensure the Leenix and Arch keyring packages are installed and populated

        # leenix:requires-sudo=true

        if leenix-pkg-missing leenix-keyring ||
          ...
          # the full set of packages in leenix-update-system-pkgs right after this (and it needs latest keyring)!

          sudo pacman -Sy
          leenix-pkg-add leenix-keyring
        ...
        sudo pacman -Sy --noconfirm archlinux-keyring \
          >/dev/null \
          2> >(grep -vE '^warning: archlinux-keyring-[^ ]+ is up to date -- reinstalling$' >&2)
