  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Set and show lightweight desktop notification reminders

        # leenix:args= [message] | show | clear

        # leenix:examples=leenix reminder 5 | leenix reminder 30 "Check the oven" | leenix reminder show | leenix reminder clear
        ...
        show_reminders() {
          ...
          local reminder_dir="''${XDG_RUNTIME_DIR:-/tmp}/leenix-reminders"
          local reminder_message=""

          timers=$(systemctl --user list-timers --all --no-legend --no-pager "leenix-reminder-*.timer" 2>/dev/null | awk '{ print $(NF - 1) }')
          ...
          for timer in $timers; do
            ...
            reminder=''${reminder#leenix-reminder-}
            set_at=''${reminder##*-}
          ...
          if [[ -z $body ]]; then
            leenix-notification-send "󰔛" "Upcoming reminders" "No outstanding reminders" -u low
          else
            leenix-notification-send "󰔛" "Upcoming reminders" "''${body%$'\n'}" -u low
          fi
        }
        ...
        clear_reminders() {
          ...
          units=$(systemctl --user list-timers --all --no-legend --no-pager "leenix-reminder-*.timer" 2>/dev/null | awk '{ print $(NF - 1), $NF }')

          if [[ -n $units ]]; then
            xargs -r systemctl --user stop <<<"$units" || true
          fi

          rm -f "$reminder_dir"/leenix-reminder-*.message 2>/dev/null || true
          leenix-notification-send "󰔛" "All reminders have been cleared" -u low
        }
        ...
        if [[ -z $minutes ]] || [[ ! $minutes =~ ^[0-9]+$ ]] || ((minutes == 0)); then
          echo "Usage: leenix-reminder  [message]"
          echo "       leenix-reminder show"
          echo "       leenix-reminder clear"
          exit 1
        fi
        ...
        unit="leenix-reminder-''${minutes}m-$set_at"
        reminder_dir="''${XDG_RUNTIME_DIR:-/tmp}/leenix-reminders"
        ...
        systemd-run --user --quiet --collect --on-active="''${minutes}m" --unit="$unit" \
          bash -c 'leenix-notification-send "󰔛" "Reminder" "$1" -u critical; rm -f "$2"' bash "$message" "$message_file"

        leenix-notification-send "󰔛" "$confirmation_title" "$confirmation" -u low
      '';
    })
  ];
}