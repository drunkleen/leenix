...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix";
      excludeShellChecks = [ "SC2016" "SC2034" "SC2128" "SC2178" ];
      ...
      text = ''
        ...
        LEENIX_BIN_DIR=$(cd -- "$(dirname -- "''${BASH_SOURCE[0]}")" && pwd)
        ...
        GROUP_DESCRIPTIONS[branch]="Leenix git branch management"
        ...
        GROUP_DESCRIPTIONS[channel]="Leenix release channel management"
        ...
        GROUP_DESCRIPTIONS[dev]="Leenix development tools"
        ...
        GROUP_DESCRIPTIONS[menu]="Leenix menu commands"
        ...
        GROUP_DESCRIPTIONS[restart]="Restart Leenix components"
        ...
        GROUP_DESCRIPTIONS[toggle]="Toggle Leenix features"
        ...
        register_command() {
          ...
          while IFS= read -r line && (( line_count < METADATA_SCAN_LIMIT )); do
            ...
            if [[ $line =~ ^[[:space:]]*#[[:space:]]*leenix:([[:alnum:]_-]+)=(.*)$ ]]; then
              ...
              *)
              ...
              if [[ -z $fallback_summary || $fallback_summary == leenix:* ]]; then
                fallback_summary=""
              fi
            fi
          done <"$file"
        
          local stem="''${file_binary#leenix-}"
          local fallback_group="$stem"
          local fallback_name=""
          local fallback_route="leenix ''${stem//-/ }"
        ...
        load_commands() {
          ...
          for file in "$LEENIX_BIN_DIR"/leenix-*; do
        ...
        load_command_by_binary() {
          local binary="$1"
          local file="$LEENIX_BIN_DIR/$binary"
        ...
        load_child_commands_by_binary() {
          ...
          for file in "$LEENIX_BIN_DIR/$binary"-*; do
            [[ -f $file && -x $file ]] || continue
            register_command "$file"
          done
        }
        ...
        group_has_child_commands() {
          ...
          for file in "$LEENIX_BIN_DIR/leenix-$group"-*; do
        ...
        group_has_binary_commands() {
          ...
          [[ -x $LEENIX_BIN_DIR/leenix-$group ]] && return 0
        ...
        load_group_commands() {
          ...
          if [[ -f $LEENIX_BIN_DIR/leenix-$group && -x $LEENIX_BIN_DIR/leenix-$group ]]; then
            register_command "$LEENIX_BIN_DIR/leenix-$group"
          fi
        ...
        resolve_direct_route() {
          ...
          for (( prefix_count = argc; prefix_count >= 1; prefix_count-- )); do
            route="leenix $(join_words " " "''${args[@]:0:prefix_count}")"
        
            binary="leenix-$(join_words "-" "''${args[@]:0:prefix_count}")"
            candidate="$LEENIX_BIN_DIR/$binary"
        ...
        fallback_group_for_key() {
          local key="$1"
          local fallback="''${COMMAND_FALLBACK_ROUTE[$key]#leenix }"
        ...
        show_main_help() {
          cat <<'EOF'
        Leenix command center
        
        Usage:
          leenix <command> [args...]
          leenix commands [--all] [--json] [--check]
          leenix <group> --help
        ...
        Common commands:
          ...
          leenix screenshot          Take a screenshot
          leenix debug               Print debugging information
        ...
        Discovery:
          leenix commands            List all commands
          leenix commands --all      Include commands explicitly marked hidden
          leenix commands --json     Machine-readable command list
          leenix commands --check    Validate command metadata and routes
        EOF
        }
        ...
        show_commands_help() {
          cat <<'EOF'
        Usage:
          leenix commands [--all] [--json] [--markdown] [--check]
        
        List commands known to the Leenix command center.
        ...
        show_commands() {
          ...
          if [[ $include_all == "true" ]]; then
            echo "Leenix commands (all):"
          else
            echo "Leenix commands:"
          fi
        ...
        show_commands_check() {
          ...
          for key in "''${COMMAND_KEYS[@]}"; do
            ...
            if [[ ! -x $LEENIX_BIN_DIR/''${COMMAND_BINARY[$key]} ]]; then
        ...
        parse_commands_args() {
          ...
          while (( $# > 0 )); do
            ...
            --help | -h)
              ...
              ;;
            *)
              echo "Unknown option for leenix commands: $1" >&2
        ...
        show_group_help() {
          ...
          if [[ -n $rows ]]; then
            ...
            echo "  No documented commands found. Try: leenix commands --all"
          fi
        }
        ...
        dispatch_fast_or_help() {
          ...
          if resolve_direct_route "$#" "''${args[@]}"; then
            ...
            binary_path="$LEENIX_BIN_DIR/$DIRECT_RESOLVED_BINARY"
        ...
        dispatch_or_help() {
          ...
          if resolve_route "$#" "''${args[@]}"; then
            ...
            binary_path="$LEENIX_BIN_DIR/''${COMMAND_BINARY[$key]}"
          ...
          if show_prefix_help "leenix ''${args[*]}"; then
            return 0
          fi
        
          echo "Unknown Leenix command: leenix ''${args[*]}" >&2
          suggestion=$(suggest_command "''${args[0]}")
          if [[ -n $suggestion ]]; then
            echo "Did you mean: leenix $suggestion ?" >&2
          fi
          echo "Run 'leenix commands --all' to discover available commands." >&2
          return 127
        }
