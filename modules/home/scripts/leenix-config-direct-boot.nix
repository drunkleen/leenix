...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-config-direct-boot";
      ...
      text = ''
        ...
        # leenix:summary=Add or remove an EFI boot entry for the Leenix UKI, allowing the system to boot directly
        ...
        existing_entry=$(efibootmgr | grep -E "^Boot[0-9A-Fa-f]+\*? Leenix([[:space:]]|$)" | head -1)

        if [[ -n $existing_entry ]]; then
          boot_num=$(echo "$existing_entry" | sed -n 's/^Boot\([0-9A-Fa-f]\+\).*/\1/p')

          if gum confirm "Disable direct boot (remove Leenix EFI entry)?"; then
          ...
          uki_file=$(find /boot/EFI/Linux/ -name "leenix*.efi" -printf "%f\n" 2>/dev/null | head -1)

          if [[ -z $uki_file ]]; then
            echo "Error: No Leenix UKI found in /boot/EFI/Linux/" >&2
            exit 1
          fi
