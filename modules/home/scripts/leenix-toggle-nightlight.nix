{ pkgs, ... }:

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-toggle-nightlight";
      excludeShellChecks = [ "SC2053" ];

      runtimeInputs = with pkgs; [
        procps
        util-linux
        coreutils
        gnugrep
        libnotify
        hyprland
      ];

      text = ''
        #!/bin/bash

        # leenix:summary=Toggle nightlight screen temperature

        # leenix:examples=leenix toggle nightlight

        ON_TEMP=4000
        OFF_TEMP=6000

        # Ensure hyprsunset is running

        if ! pgrep -x hyprsunset; then
          setsid uwsm-app -- hyprsunset &
          sleep 1 # Give it time to register
        fi

        # Query the current temperature

        CURRENT_TEMP=$(hyprctl hyprsunset temperature 2>/dev/null | grep -oE '[0-9]+' || true)

        restart_nightlighted_waybar() {
          if grep -q "custom/nightlight" ~/.config/waybar/config.jsonc; then
            leenix-restart-waybar # restart waybar in case user has waybar module for hyprsunset
          fi
        }

        if [[ $CURRENT_TEMP == $OFF_TEMP ]]; then
          hyprctl hyprsunset temperature $ON_TEMP
          notify-send -u low "  Nightlight screen temperature"
          restart_nightlighted_waybar
        else
          hyprctl hyprsunset temperature $OFF_TEMP
          notify-send -u low "   Daylight screen temperature"
          restart_nightlighted_waybar
        fi
      '';
    })
  ];
}