{ pkgs, ... }:

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-weather-icon";

      runtimeInputs = with pkgs; [
        jq
        coreutils
      ];

      text = ''
        #!/bin/bash

        # leenix:summary=Returns a weather condition icon, adjusted for live sunrise and sunset.

        if [[ ''${1:-} == "-" ]]; then
          weather_data=$(cat)
        else
          weather_data=$(leenix-weather-data) || exit 1
        fi

        extracted=$(printf '%s' "''$weather_data" |
          jq -er '[.current_condition[0].weatherCode, .weather[0].astronomy[0].sunrise, .weather[0].astronomy[0].sunset] |
            select(all(. != null and . != "")) |
            @tsv') || exit 1

        IFS=$'\t' read -r weather_code sunrise sunset <<< "''$extracted"

        if [[ ! ''$weather_code =~ ^[0-9]+$ ||
              ! ''$sunrise =~ ^[0-9]{1,2}:[0-9]{2}\ [AP]M$ ||
              ! ''$sunset =~ ^[0-9]{1,2}:[0-9]{2}\ [AP]M$ ]]; then
          exit 1
        fi

        now_epoch=$(date +%s)
        sunrise_epoch=$(date -d "today ''$sunrise" +%s 2>/dev/null || echo 0)
        sunset_epoch=$(date -d "today ''$sunset" +%s 2>/dev/null || echo 0)

        if (( sunrise_epoch > 0 && sunset_epoch > 0 &&
              (now_epoch < sunrise_epoch || now_epoch >= sunset_epoch) )); then
          night=true
        else
          night=false
        fi

        case "''$weather_code" in
          113) [[ ''$night == "true" ]] && icon="" || icon="" ;;
          116) [[ ''$night == "true" ]] && icon="" || icon="" ;;
          119|122) icon="" ;;
          143|248|260) icon="" ;;
          176|263|266|293|296|353) [[ ''$night == "true" ]] && icon="" || icon="" ;;
          179|227|230|323|326|368) [[ ''$night == "true" ]] && icon="" || icon="" ;;
          182|185|281|284|311|314|317|320|350|362|365|374|377) icon="" ;;
          200|386|389|392|395) icon="" ;;
          299|302|305|308|356|359) icon="" ;;
          329|332|335|338|371) icon="" ;;
          *) icon="" ;;
        esac

        printf '%s\n' "''$icon"
      '';
    })
  ];
}
