...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-plymouth-preview";
      excludeShellChecks = [ "SC2086" "SC2140" "SC2206" ];
      ...
      text = ''
        ...
        # leenix:summary=Preview a Plymouth boot screen with custom colors and logo

        # leenix:args=  <path-to-logo.png> 

        # leenix:examples=leenix plymouth preview '#1d2021' '#ebdbb2' ~/.config/leenix/current/theme/plymouth/logo.png /tmp/plymouth-preview.png

        # Render a Plymouth login-screen preview PNG by compositing the staged leenix

        # theme assets (recolored with the given text color) onto the background.

        if [[ $# -ne 4 ]]; then
          echo "Usage: leenix-plymouth-preview   <path-to-logo.png> " >&2
          exit 1
        fi
        ...
        find ~/.local/share/leenix/default/plymouth -maxdepth 1 -type f -exec cp -t "$staging_dir/" {} +
