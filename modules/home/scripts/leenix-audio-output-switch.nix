...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-audio-output-switch";
      ...
      text = ''
        ...
        # leenix:summary=Switch between audio outputs while preserving the mute status. By default mapped to Super + Mute.

        sinks=$(pactl -f json list sinks | jq '[.[] | select((.ports | length == 0) or ([.ports[]? | .availability != "not available"] | any))]')
        sinks_count=$(echo "$sinks" | jq '. | length')

        if (( sinks_count == 0 )); then
          leenix-swayosd-client --custom-message "No audio devices found"
          exit 1
        fi
