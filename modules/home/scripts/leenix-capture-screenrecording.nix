...

{
  home.packages = [
    (pkgs.writeShellApplication {
      name = "leenix-capture-screenrecording";
      excludeShellChecks = [ "SC1090" "SC2086" "SC2119" "SC2120" "SC2155" ];
      ...
      text = ''
        ...
        # leenix:summary=Start or stop screen recording
        # leenix:group=capture
        # leenix:args=[--with-desktop-audio] [--with-microphone-audio] [--with-webcam] [--webcam-device=<device>] [--resolution=<size>] [--stop-recording]
        # leenix:examples=leenix screenrecord | leenix capture screenrecord --with-desktop-audio
        # leenix:aliases=leenix screenrecord
        #
        # Env: LEENIX_SCREENRECORD_USE_PORTAL=true skips the built-in slurp picker and
        ...
        # start.
        #
        # Env: LEENIX_SCREENRECORD_DEBUG=true appends gpu-screen-recorder's stderr (and
        # the picker target it was launched with) to /tmp/leenix-screenrecord.log so
        ...
        RECORDING_FILE="/tmp/leenix-screenrecord-filename"
        LOG_FILE=$([[ ''${LEENIX_SCREENRECORD_DEBUG:-false} == "true" ]] && echo "/tmp/leenix-screenrecord.log" || echo "/dev/null")
        ...
        # Mirrors leenix-capture-screenshot so the picker UX is identical.
        get_rectangles() {
        ...
        start_screenrecording() {
          ...
          if [[ ''${LEENIX_SCREENRECORD_USE_PORTAL:-false} == "true" ]]; then
            target="portal"
