  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Transcode an image into ASCII/Unicode art text

        # leenix:group=transcode

        # leenix:name=ascii

        # leenix:args=<input-image.svg|png> <output-path> [--width <cols>] [--height <rows>] [--mode <braille|block>] [--threshold <0-100>] [--invert]

        # leenix:examples=leenix transcode ascii ~/logo.svg /tmp/logo.txt | leenix transcode ascii ~/logo.png ~/.config/leenix/branding/screensaver.txt --width 80

        set -o pipefail

        usage() {
          cat <<'EOF'
        Usage: leenix-transcode-ascii <input-image.svg|png> <output-path> [options]

        Transcodes an image into ASCII/Unicode art text.

        Options:
          -w, --width <cols>          Maximum output width in terminal columns (default: 80)
          -H, --height <rows>         Maximum output height in terminal rows (default: 26)
          -m, --mode <braille|block>  Output style (default: braille)
          -t, --threshold <0-100>     Pixel threshold from 0-100 (default: 50)
        ...
        if leenix-cmd-missing magick; then
