  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Toggle passwordless sudo for the current user.

        # leenix:args=[MINUTES]

        # leenix:requires-sudo=true

        NOPASSWD_FILE="/etc/sudoers.d/99-leenix-nopasswd-''${USER}"
        TIMER_NAME="leenix-nopasswd-expire-''${USER}"

        MINUTES=''${1:-15}
        if [[ ''${1:-} && ! $1 =~ ^[0-9]+$ ]]; then
          echo "Usage: leenix-sudo-passwordless [MINUTES]" >&2
          exit 1
        fi
        ...
        if sudo test -f "$NOPASSWD_FILE"; then
          ...
          if gum confirm "Enable passwordless sudo for ''${MINUTES} minutes? This is a significant security risk!"; then
            ...
            echo "Note: if you restart before then, run leenix-sudo-passwordless again to disable it."
          else
