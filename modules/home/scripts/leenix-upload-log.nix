  ...
  home.packages = [
    (pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # leenix:summary=Upload logs to logs.leenix.org
        ...
        case "$LOG_TYPE" in
          install)
            ...
            # leenix install log lives at /mnt/var/log/leenix-install.log. On the
            # installed system it's at /var/log/leenix-install.log. Prefer whichever
            ...
            if [[ -s /mnt/var/log/leenix-install.log ]]; then
              LEENIX_LOG="/mnt/var/log/leenix-install.log"
            else
              LEENIX_LOG="/var/log/leenix-install.log"
            fi
            ...
            if [[ -s $LEENIX_LOG ]]; then
              printf '\n========================================\nLEENIX INSTALL LOG (%s)\n========================================\n\n' "$LEENIX_LOG" >>"$TEMP_LOG"
              cat "$LEENIX_LOG" >>"$TEMP_LOG"
            else
              echo "Warning: leenix install log not found (looked at /mnt/var/log/ and /var/log/)" >&2
            fi
            ...
            echo "Uploading installation log to logs.leenix.org..."
            ;;
          ...
          this-boot)
            ...
            echo "Uploading current boot logs to logs.leenix.org..."
            ;;
          ...
          last-boot)
            ...
            echo "Uploading previous boot logs to logs.leenix.org..."
            ;;
          ...
          installed | system-info)
            ...
            echo "Uploading system information to logs.leenix.org..."
            ;;
        ...
        URL=$(curl -sf -F "file=@$TEMP_LOG" https://logs.leenix.org/)
