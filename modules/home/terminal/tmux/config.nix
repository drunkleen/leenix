...
# LEENIX tmux — Omarchy parity, non-theme portion.
#
# This file is a faithful port of Omarchy's config/tmux/tmux.conf (quattro),
# preserving every keybinding and general option exactly. The ONLY adaptations
# are:
#   - omarchy-menu-tmux-keybindings -> leenix-menu-tmux-keybindings (LEENIX
#     helper naming; same behavior)
#   - screen-256color -> tmux-256color / history 5000 -> 50000 (restore Omarchy)
#   - clock24 (non-Omarchy HM option) removed (tmux default is already 24h)
#
# Colors live in theme.nix (LEENIUM); nothing here is theme-specific.
let
...
rec {
  ...
  configAndHelp = ''
    ...
    bind -N "Show Tmux keybindings" ? display-popup -E -w 80% -h 70% -T "Tmux keybindings" "leenix-menu-tmux-keybindings --print | less -R"
  '';
  ...
  # Status bar (structure only; colors in theme.nix)
  status = ''
