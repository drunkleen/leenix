...
M.get_mappings = function(mappings, tb_to_add)
  local excluded_groups = require("tiredconfig").cheatsheet.excluded_groups
...
M.organize_mappings = function()
  ...
  for _, mode in ipairs(modes) do
    ...
    require("tired.cheatsheet").get_mappings(keymaps, tb_to_add)

    local bufkeymaps = vim.api.nvim_buf_get_keymap(0, mode)
    require("tired.cheatsheet").get_mappings(bufkeymaps, tb_to_add)
  end
...
M.autocmds = function(buf)
  require("tired.utils").set_cleanbuf_opts("nvcheatsheet", buf)
  ...
  api.nvim_create_autocmd({ "WinResized", "VimResized" }, {
    group = group_id,
    callback = function()
      require("tired.cheatsheet." .. config.cheatsheet.theme)(vim.g.nvch_buf, vim.g.nvch_win, "redraw")
    end,
  })
